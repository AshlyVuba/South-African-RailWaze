...
export const MemoryVaultSlider: React.FC<MemoryVaultSliderProps> = ({
  ...
  // Issue #39 "Use of Quantum Tech": the *displayed* order of trivia answer
  ...
  useEffect(() => {
    ...
    if (!triviaQuestion?.options?.length) {
      setShuffledOptions([]);
      return;
    }

    quantumShuffle(triviaQuestion.options).then((result) => {
      if (!cancelled) {
        setShuffledOptions(result.items);
        setRandomSource(result.source);
      }
    });
              ...
              <div className="mt-3 border-t border-white/10 pt-3">
                ...
                {triviaQuestion && (
                    <div aria-live="polite" className="mt-3">
                      ...
                      <p
                          className="mt-1 text-[10px] font-mono uppercase tracking-widest text-neutral-500"
                          title="Answer order for this question was randomized using this entropy source"
                      >
                        {randomSource === 'quantum'
