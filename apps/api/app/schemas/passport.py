...
class PassportState(BaseModel):
    ...
    collected_stamps: list[PassportStamp] = Field(default_factory=list, alias="collectedStamps")
    stamps: list[PassportStamp] | None = None
    completed_trivia_ids: list[str] = Field(default_factory=list, alias="completedTriviaIds")

    @model_validator(mode="after")
    def sync_compatibility_fields(self):
        if self.rank is None:
            self.rank = self.current_rank
        elif self.current_rank == TravelerRank.STOKER and self.rank != TravelerRank.STOKER:
